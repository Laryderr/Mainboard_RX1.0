...
void DJI_Update(DJI_t *motor, uint8_t *fdbData)
{
	...
	if (motor->Calculate.RotorAngle_0_360_Log[NOW] - motor->Calculate.RotorAngle_0_360_Log[LAST] > (180.0f))
		...
		motor->Calculate.RotorRound++;
	/*解释：
			假设电机从 350 度 转到了 10 度，按值计算：

			第一种情况：

			上一次记录的角度为 350 度，现在的角度为 10 度。
			角度差：10 - 350 = -340 度，这个值小于 -180 度。
			因为电机从高角度（接近 360 度）转到了低角度（接近 0 度），表示电机实际增加了一圈，所以圈数增加，RotorRound++。
			第二种情况：

			上一次记录的角度为 10 度，现在的角度为 350 度。
			角度差：350 - 10 = 340 度，这个值大于 180 度。
			这表示电机从低角度快速转到了高角度，意味着电机减少了一圈（从 360 度回到了 0 度），所以圈数减少，RotorRound--。*/
...
HAL_StatusTypeDef DJI_CanMsgDecode(uint32_t Stdid, uint8_t *fdbData)
{
	...
	if (i >= 0 && i < 8)
	{
		// 上电后进行反馈数据计数，计数到第50次时设置为当前偏差（电机以后的总转角从这里开始计数）
